...
def training_parameters(plan_path: Path, dataset: DatasetRecord, display_name: str,
    ...
    return {
        ...
        # Embedded Python ignores PYTHONUTF8; -X also propagates to spawned workers.
        "gpt_command": [str(config.python_path.resolve()), "-X", "utf8", "-s", "GPT_SoVITS/s1_train.py",
                        "--config_file", str(s1path)],
        "gpt_cwd": root,
        "gpt_outputs": [gpt_weight],
        "gpt_weight_path": gpt_weight,
        "gpt_timeout": 7200,
        "sovits_command": [str(config.python_path.resolve()), "-X", "utf8", "-s", "GPT_SoVITS/s2_train.py",
                           "--config", str(s2path)],
        "sovits_cwd": root,
        "sovits_outputs": [],
