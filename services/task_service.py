...
_PROCESS_LOCK_PATH = PROJECT_ROOT / "data" / ".gpu-process.lock"
...
def _runtime_task(record: TaskRecord) -> TaskRecord:
    if record.log_path:
        return record.model_copy(update={"log_path": resolve_project_path(record.log_path, root=PROJECT_ROOT)})
    return record
...
def try_acquire_gpu(*, kind: str = "other") -> None:
    ...
    try:
        ...
        handle = _PROCESS_LOCK_PATH.open("a+b")
        handle.seek(0)
        try:
            msvcrt.locking(handle.fileno(), msvcrt.LK_NBLCK, 1)
        except OSError as exc:
            handle.close()
            raise AppError("GPU_BUSY", "已有模型任务正在其他进程执行，请等待它结束。") from exc
    ...
    try:
        ...
        for item in files:
            ...
            if header.startswith('{"event":'):
                events = [json.loads(line) for line in item.read_text(encoding="utf-8").splitlines()]
                final = events[-1]
                stages.append(["synthesis", "0" if final["event"] == "succeeded" else final.get("error_code", "执行中"),
                               final["event"], item.name])
                continue
...
def recovery_notice(index_path: Path = TASK_INDEX) -> str:
    try:
        records = list_tasks(index_path, strict=False)
    except Exception:
        return "任务恢复检查失败，请查看 data/logs。"
    recovered = [item for item in records if item.status == TaskStatus.FAILED and "服务重启后任务" in item.message]
    if not recovered:
        return ""
    latest = sorted(recovered, key=lambda item: item.updated_at, reverse=True)[:3]
    lines = ["上次运行有任务被中断，已标记为失败："]
    for item in latest:
        detail = f"{item.task_id}（{item.kind}）：{item.message}"
        if item.log_path:
            detail += f"；日志：{relative_path(item.log_path)}"
        lines.append("- " + detail)
    lines.append("请检查日志后重新提交，不会自动重放。")
    return "\n".join(lines)
