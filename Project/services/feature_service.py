...
def feature_status(dataset_id):
    ...
    if not record or not record.feature_manifest:
        return "尚未提取特征。请先保存文本校对。"
    return f"特征清单：{dataset_service.relative_path(record.feature_manifest)}\n特征已保存；尚未执行新训练。"
...
def extract_features(dataset_id, *, task_id=None, config_path=LOCAL_CONFIG,
    ...
    current = None
    try:
        current = task_service.transition(TaskRecord(task_id=task_id, kind="extract_features"),
