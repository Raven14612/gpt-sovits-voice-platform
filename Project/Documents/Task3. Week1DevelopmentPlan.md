...
**怎么读对接：** 先看 [§0.6 节点依赖、等待与并行](#06-节点依赖等待与并行必读)（含硬等待/软依赖/可并行定义、全员矩阵、建议日历）；再看自己章节的节点总览表；每个节点标题下还有一行 **对接关系**（谁等你、你等谁、能否隔离开工）。
...
| 术语 | 含义 | 你该怎么做 |
|---|---|---|
| **硬等待** | 没有对方交付物，本节点无法验收或会做错 | 停下来催对方，或先做同角色里不依赖它的节点 |
| **软依赖** | 有草稿就能开工；定稿后要对照修订 | 先按 Task1 / 暂定契约做，对方交付后做一次对齐 |
| **可并行** | 两边工作面隔离，互不等待 | 同时推进，只在约定交接点同步 |
| **本角色内串行** | 只依赖自己的上一节点 | 按自己的 N1→N6 顺序做即可 |

主链路（箭头 = 硬等待或强交付）：

```text
CXY-N1 版本表模板 ──► LJQ-N1 / LHY-N1（可先按模板开工；签字版用于验收）
CXY-N2 ──► CXY-N3 / CXY-N4（队长内部）
CXY-N3 契约冻结 ──硬等──► LHY-N2、LHY-N3、WGX-N3（真对接）
CXY-N4 回归文本 ──硬等──► LJQ-N4、WGX-N4
CXY-N5 授权策略 ──软──► LJQ-N2、LHY-N3（采集/上传前对齐授权文案）
LJQ-N1 9880 可访问 ──硬等──► LHY-N4（真实代理；mock 单测可先行）
LJQ-N2 → LJQ-N3 → LJQ-N4 → LJQ-N5 → LJQ-N6（算法内部串行，与前端早期隔离）
LHY-N3 上传 API ──硬等──► WGX-N3（真上传；页面壳可先做）
LHY-N4 + LHY-N6 ──硬等──► WGX-N6（真联调）
LJQ-N4 + LHY-N4 + WGX-N6 ──硬等──► CXY-N6（周五验收）
```

#### 0.6.1 谁可以立刻并行（彻底隔离段）

以下工作面互不等待，应尽早并行，不要互相挡着：

| 并行组 | 参与节点 | 说明 |
|---|---|---|
| 开工并行组 | CXY-N1/N2，LJQ 读官方文档准备，LHY-N1，WGX-N1/N2 | 队长写规范，后端搭空壳，前端搭路由，算法装环境——互不阻塞 |
| 规范与算法并行 | CXY-N3/N4/N5 与 LJQ-N2/N3 | 契约/标签起草时，算法可同时采音、切分、出 `.list`；算法本周不接业务 API |
| 算法听测与后端实现并行 | LJQ-N4/N5 与 LHY-N2/N3/N4（mock）与 WGX-N4/N5 | LJQ 走官方 9880 出样音；LHY 可用 mock 写代理；WGX 可先做表单壳 |
| 前端壳与后端字段并行 | WGX-N1～N5（除真对接）与 LHY-N1～N5 | 真上传/真 TTS 前，前端可用本地 mock；不要空等后端每一行代码 |

#### 0.6.2 全员节点等待矩阵

「我硬等」= 没有它就不能完成本节点验收。「谁硬等我」= 你做完之前对方不能进入其验收。「可并行」= 建议同时做。

| 节点 | 我硬等 | 软依赖（可不挡开工） | 可与谁并行 | 谁硬等我（我解锁谁） |
|---|---|---|---|---|
| CXY-N1 | 无 | LJQ/LHY 事后回填实测 | LHY 读目录、WGX-N1、LJQ 装驱动 | LJQ-N1、LHY-N1 需要模板；全员环境验收需要签字版 |
| CXY-N2 | CXY-N1（可与 N1 尾声重叠） | 无 | LJQ-N1、LHY-N1、WGX-N1/N2 | CXY-N3、CXY-N4 |
| CXY-N3 | CXY-N2 | 可对照 `api_v2.py` 先起草 | LJQ-N2/N3、WGX-N1/N2、LHY-N1 | **LHY-N2/N3、WGX-N3**（真对接） |
| CXY-N4 | CXY-N2 | 可与 CXY-N3 并行起草 | CXY-N3、LJQ-N2/N3、LHY-N1、WGX-N1/N2 | **LJQ-N4、WGX-N4** |
| CXY-N5 | CXY-N3（建议）；紧急时可按 Task1 先出草稿 | 无 | LJQ-N3/N4、LHY-N2、WGX-N2 | LJQ-N2 定稿、LHY-N3 授权字段（软→对齐） |
| CXY-N6 | **LJQ-N4、LHY-N4、WGX-N6** | LJQ-N5、LHY-N6、各文档 | 无（收口） | 无（本周终点） |
| LJQ-N1 | CXY-N1 模板（至少有待填表） | 签字版可后补实测 | CXY-N2、LHY-N1、WGX-N1/N2 | **LHY-N4** 真实推理；CXY 环境验收 |
| LJQ-N2 | 无（可先采音） | **CXY-N5** 授权文案定稿前不得宣称数据合规完成 | CXY-N3/N4、LHY-N1/N2、WGX-N1/N2 | LJQ-N3；路径信息软交给 LHY |
| LJQ-N3 | LJQ-N2 | 无 | **与 LHY/WGX 彻底隔离**；与 CXY-N3～N5 并行 | LJQ-N4/N6；`.list` 供第 2 周，本周不挡前端 |
| LJQ-N4 | **LJQ-N1、CXY-N4** | 无 | LHY-N2～N4（mock）、WGX-N3～N5 | CXY-N6；样音软支撑 LHY-N4 对照 |
| LJQ-N5 | LJQ-N4 | 无 | LHY-N5/N6、WGX-N5、**与前端无硬依赖** | CXY-N6（证据）；听感说明软给 WGX |
| LJQ-N6 | LJQ-N3～N5（可先出半成品） | 无 | WGX-N5、CXY 整理文档 | LHY/WGX 联调质量；**不硬挡** LHY-N4 开工 |
| LHY-N1 | 无（CXY-N1 为软：目录与版本约定） | CXY-N1 | LJQ-N1、CXY-N2、WGX-N1/N2 | 无硬解锁；给 WGX 一个可连的空服务更佳 |
| LHY-N2 | **CXY-N3** | 可按 Task1 §6 先建表草稿 | LJQ-N2/N3、WGX-N1/N2 | LHY-N5 |
| LHY-N3 | **CXY-N3** | CXY-N5 授权策略 | LJQ-N3/N4、WGX 做上传壳 | **WGX-N3** 真上传 |
| LHY-N4 | **CXY-N3、LJQ-N1**（真 9880） | LJQ-N4/N6 参数；**mock 单测可先于 LJQ-N1** | LJQ-N4/N5、WGX-N4/N5 | **WGX-N6**；CXY-N6 |
| LHY-N5 | LHY-N2、LHY-N4 | 无 | LJQ-N5、WGX-N4/N5 | WGX-N6（轮询） |
| LHY-N6 | LHY-N3～N5 | 无 | LJQ-N5/N6、WGX-N5 | **WGX-N6** |
| WGX-N1 | 无 | 无 | **与三人彻底隔离**，立即开工 | WGX-N2 |
| WGX-N2 | WGX-N1 | CXY-N3 状态枚举（可先按 Task1 写） | CXY-N2～N4、LHY-N1、LJQ-N1/N2 | WGX-N3；字段需求软给 LHY |
| WGX-N3 | **CXY-N3**；真联调还硬等 **LHY-N3** | 页面壳+本地 mock 可先做 | LJQ-N3/N4、LHY-N4 | WGX-N6 路径上的一环 |
| WGX-N4 | **CXY-N4**（标签与回归文本） | CXY-N3 / LHY 契约 | LHY-N3/N4、LJQ-N4 | WGX-N5 |
| WGX-N5 | WGX-N4 | LHY output 字段 | LHY-N5/N6、LJQ-N5 | WGX-N6 |
| WGX-N6 | **LHY-N4、LHY-N6** | LJQ 样音作演示对照 | 无（联调收口） | **CXY-N6** |

#### 0.6.3 建议日历（谁先动、谁在等）

| 时段 | CXY | LJQ | LHY | WGX |
|---|---|---|---|---|
| 周初 Day1–2 | N1→N2→开写 N3/N4 | N1 装环境；N2 可开始采音 | N1 骨架（不等契约定稿） | N1→N2（完全独立） |
| Day2–3 | **冻结 N3**；完成 N4；开写 N5 | N2→N3（与前后端隔离） | **拿到 N3 后**做 N2→N3 | 壳继续；**等 N3+LHY-N3** 再真上传 |
| Day3–4 | N5 定稿；跟踪阻塞 | **等 N4 文本后**跑 N4→N5 | **等 LJQ-N1** 做真代理 N4→N5 | N4 表单（等 CXY-N4）；N5 播放器 |
| Day4–5 | 准备验收清单 | N6 交接 | N6 OpenAPI | **等 LHY-N4/N6** 做 N6 |
| 周五 | **N6 验收**（硬等三人证据） | 到场带 WAV/日志 | 到场带 API/测试 | 到场带浏览器记录 |

一句话口令：

- **CXY 前半周是关键路径**：N3、N4 不定，LHY 真接口、WGX 真对接、LJQ 正式零样本听测都会堵。
- **LJQ 与 WGX 前半周大多可各干各的**；LJQ 出音不依赖前端，前端搭壳不依赖算法。
- **LHY 是中间汇合点**：左等契约与 9880，右挡前端真联调。
- **唯一全员硬汇合点是周五 CXY-N6**。
...
| 节点 | 我硬等 | 可并行 / 隔离说明 | 产出 | 我解锁谁 |
|---|---|---|---|---|
| CXY-N1 环境与版本冻结 | 无（立刻开工） | 与 LJQ 装驱动、LHY-N1、WGX-N1 并行；实测值事后由 LJQ/LHY 回填 | `Documents/Week1/version-baseline.md` | LJQ-N1、LHY-N1（模板） |
| CXY-N2 需求与证据矩阵 | CXY-N1 | 与 LJQ-N1、LHY-N1、WGX-N1/N2 **隔离并行** | `requirement-evidence-matrix.md` | CXY-N3、CXY-N4 |
| CXY-N3 接口契约 v0.1 | CXY-N2 | 与 LJQ-N2/N3、WGX 壳 **隔离并行**；勿等算法出音 | `api-contract-v0.1.md` | **LHY-N2/N3、WGX-N3** |
| CXY-N4 情感标签与回归文本 | CXY-N2（可与 N3 并行） | 与 LHY-N1、LJQ-N2/N3、WGX-N1/N2 并行 | `emotion-and-regression-texts.md` | **LJQ-N4、WGX-N4** |
| CXY-N5 安全策略初稿 | 建议等 CXY-N3 | 与 LJQ-N3/N4、LHY-N2 并行 | `safety-policy-draft.md` | LJQ-N2 / LHY-N3（授权对齐，软） |
| CXY-N6 联调主持与本周验收 | **LJQ-N4 + LHY-N4 + WGX-N6** | 无；收口，不要提前签字 | `week1-acceptance.md` | 无（本周终点） |

> 队长节奏：Day1–2 完成 N1/N2 并推进 N3/N4；**N3、N4 是全队阻塞点**，优先冻结。N6 之前不要空等——用时间追进度和风险，而不是替别人写代码。

### 1.3 逐节点工作与提示词

#### CXY-N1 环境与版本冻结

**对接关系：** 立刻开工，不等他人。发模板后 LJQ/LHY 各自回填实测（软）；你不替他们装环境。解锁 LJQ-N1、LHY-N1 用同一张表。与 WGX-N1 无关系。
...
**对接关系：** 只硬等自己的 N1。与 LJQ-N1、LHY-N1、WGX-N1/N2 彻底并行，互不等待。做完后解锁你自己的 N3、N4；他人此时仍可不看此文档继续搭环境/壳。
...
**对接关系：** 硬等 CXY-N2。冻结前 LHY 只做 N1/草稿，WGX 只做壳；**冻结当日起** LHY-N2/N3、WGX-N3 才能真对接。与 LJQ-N2/N3 隔离并行——算法出 `.list` 不等你的契约。这是全队最大阻塞点之一，优先完成。
...
**对接关系：** 硬等 CXY-N2，**可与 N3 并行**。做完前 LJQ 不能开始正式 N4 听测、WGX 不能定情感下拉文案。与 LHY 早期实现隔离——后端本周只原样存 JSON。尽快发出 5 条固定文本。
...
**对接关系：** 建议硬等 N3 字段稳定后再定授权字段名；紧急时可按 Task1 先出草稿。LJQ-N2 可先采音，但合规结论等你定稿；LHY-N3 上传授权文案与你对齐（软）。**不阻塞** LJQ-N4 出 WAV、不阻塞 WGX 搭壳。本周不要求 LHY 实现水印。
...
**对接关系：** **硬等三人证据齐**：LJQ-N4（+建议 N5）、LHY-N4（+建议 N6）、WGX-N6。此前不要写“通过”。验收顺序：算法样音 → 后端代理 → 前端浏览器。任一层缺文件就记未通过/部分通过，禁止用 Mock 顶替。
...
| 节点 | 我硬等 | 可并行 / 隔离说明 | 产出 | 我解锁谁 |
|---|---|---|---|---|
| LJQ-N1 官方环境冒烟 | CXY-N1 模板（至少待填表） | 与 CXY-N2、LHY-N1、WGX-N1/N2 **隔离并行** | 环境实测 + 启动记录 | **LHY-N4**（真 9880） |
| LJQ-N2 授权音频规范与采集 | 无（可先采） | 与 CXY-N3/N4、LHY/WGX 早期工作并行；**CXY-N5 定稿前不宣称合规完成** | 8–15 分钟音频 + 质量报告 | LJQ-N3 |
| LJQ-N3 官方数据准备 | LJQ-N2 | **与后端/前端彻底隔离**，无需等 API 或页面 | `.list` + 抽检 | LJQ-N4/N6（第 2 周训练更依赖） |
| LJQ-N4 零样本连续 3 次 | **LJQ-N1 + CXY-N4** | 与 LHY mock 开发、WGX 表单壳并行；走官方 9880，**不等业务后端** | 3 个 WAV + 日志 | **CXY-N6**；软支撑 LHY 对照 |
| LJQ-N5 基线音色—韵律对照 | LJQ-N4 | 与 LHY-N5/N6、WGX-N5 并行；**不依赖前端** | 对照样音 + 听测表 | CXY-N6（证据） |
| LJQ-N6 算法交接契约 | LJQ-N3～N5 | 可先出半成品；**不硬挡** LHY-N4 用 mock/日志开工 | `algorithm-handoff.md` | LHY/WGX 联调质量 |

> 算法口令：前半周 N1→N3 尽量自己跑通；**唯一会卡住你的外人是 CXY-N4（回归文本）**；你卡住别人的是 N1（9880）和 N4（验收样音）。不要等前端。

### 2.3 逐节点工作与提示词

#### LJQ-N1 官方环境冒烟

**对接关系：** 硬等 CXY-N1 至少有一张待填表；可与 CXY-N2、LHY-N1、WGX-N1/N2 并行。你做完并保持 9880 可访问后，**LHY-N4 才能做真实代理**（其 mock 单测可不等等你）。与前端无对接。
...
**对接关系：** **可立刻采音**，不等后端/前端。软等 CXY-N5：授权策略未定稿前不要对外宣称“合规数据已完成”。与 CXY-N3/N4、LHY 骨架、WGX 壳隔离并行。路径与质量规则事后交给 LHY 对照校验即可。
...
**对接关系：** 只硬等自己的 N2。**与 LHY/WGX 本周彻底隔离**——切分/ASR/`.list` 不依赖业务 API 或页面。做完主要解锁自己的 N4/N6；`.list` 给 LHY 记路径即可，本周不挡前端。
...
**对接关系：** 硬等 **自己的 N1（9880）+ CXY-N4（固定文本）**。走官方端口，**不要等 LHY 代理或 WGX 页面**。可与 LHY-N2～N4（含 mock）、WGX-N3～N5 并行。你的 3 个 WAV 是 CXY-N6 硬证据，也是 LHY 对照的软参考。
...
**对接关系：** 只硬等自己的 N4。与 LHY-N5/N6、WGX-N5 并行；**不依赖也不等待前端**。听测结论软交给 WGX 做控件提示；硬证据交给 CXY-N6。
...
**对接关系：** 依赖自己的 N3～N5（可先交半成品）。**不硬挡 LHY-N4**：后端可用你的 N1/N4 日志先联调。完整 handoff 提升 LHY/WGX 联调质量，周五前必须齐，供 CXY 归档。
...
| 节点 | 我硬等 | 可并行 / 隔离说明 | 产出 | 我解锁谁 |
|---|---|---|---|---|
| LHY-N1 项目骨架 | 无（CXY-N1 为软依赖） | 与 LJQ-N1、CXY-N2、WGX-N1/N2 **隔离并行** | 可启动 backend + health | 无硬解锁；尽早给 WGX 空地址 |
| LHY-N2 SQLite 最小模型 | **CXY-N3** | 契约未冻前可按 Task1 §6 打草稿；与 LJQ-N2/N3、WGX 壳并行 | Consent/Voice/TTSJob/Audit | LHY-N5 |
| LHY-N3 上传 / 授权 / 路径白名单 | **CXY-N3** | CXY-N5 软对齐；与 LJQ-N3/N4、WGX 上传壳并行 | `POST /api/audio/upload` | **WGX-N3** 真上传 |
| LHY-N4 零样本代理 | **CXY-N3 + LJQ-N1**（真链路） | **mock 单测可先于 LJQ-N1**；与 LJQ-N4、WGX-N4/N5 并行 | adapter + `POST /api/tts` | **WGX-N6、CXY-N6** |
| LHY-N5 任务状态机 | LHY-N2、LHY-N4 | 与 LJQ-N5、WGX-N4/N5 并行 | `GET /api/tasks/{id}` | WGX-N6 轮询 |
| LHY-N6 测试与 OpenAPI | LHY-N3～N5 | 与 LJQ-N6、WGX-N5 并行 | pytest + OpenAPI 示例 | **WGX-N6** |

> 后端口令：你是汇合点——**左等 CXY-N3 与 LJQ-N1，右挡 WGX-N3/N6**。契约未冻结前只做 N1 和按 Task1 的草稿；不要空等算法 `.list`（本周训练不做）。

### 3.3 逐节点工作与提示词

#### LHY-N1 项目骨架

**对接关系：** 可立刻开工；CXY-N1 为软依赖（目录/版本约定）。与 LJQ-N1、CXY-N2、WGX-N1/N2 隔离并行。**不要等契约定稿才建仓库。** 尽早把 health 地址告诉 WGX（哪怕还是空壳）。
...
**对接关系：** **硬等 CXY-N3** 才能定枚举与字段名；未冻结前可按 Task1 §6 打草稿，冻结后必须对齐修订。与 LJQ-N2/N3、WGX 壳并行。主要解锁自己的 N5，不直接挡前端。
...
**对接关系：** **硬等 CXY-N3**；CXY-N5 授权文案软对齐。与 LJQ-N3/N4、WGX 上传壳并行。你交付后 **WGX-N3 才能做真上传联调**（其 UI 壳可已先做好）。不等 LJQ 的 `.list`。
...
**对接关系：** 真链路硬等 **CXY-N3 + LJQ-N1（9880）**。**mock 单测可先于 LJQ-N1**，不要干等。LJQ-N4/N6 为软参考。与 LJQ 听测、WGX-N4/N5 并行。你做完后与 N6 一起硬解锁 **WGX-N6**，并作为 CXY-N6 证据之一。
...
**对接关系：** 硬等自己的 N2、N4。与 LJQ-N5、WGX-N4/N5 并行；前端此时可继续做播放器壳。状态枚举稳定后 WGX-N6 才能正确轮询。
...
**对接关系：** 硬等自己的 N3～N5。与 LJQ-N6、WGX-N5 并行。**OpenAPI/示例是 WGX-N6 的硬前置**——未交付前前端不要宣称联调完成。摘要交给 CXY。
...
| 节点 | 我硬等 | 可并行 / 隔离说明 | 产出 | 我解锁谁 |
|---|---|---|---|---|
| WGX-N1 脚手架与信息架构 | **无，立刻开工** | **与 CXY/LJQ/LHY 彻底隔离** | Vue3 工程 + 路由 | WGX-N2 |
| WGX-N2 低保真原型与组件清单 | WGX-N1 | 状态枚举可先按 Task1；与全员前半周并行 | 原型 + 状态清单 | WGX-N3；需求软给 LHY |
| WGX-N3 上传与授权页 | **CXY-N3**；真联调再硬等 **LHY-N3** | 页面壳 + mock 可先做，**不必空等后端每一天** | 上传授权流程 | WGX-N6 路径 |
| WGX-N4 TTS 合成表单壳 | **CXY-N4** | 与 LHY-N3/N4、LJQ-N4 并行；可先接 mock | 合成页占位 | WGX-N5 |
| WGX-N5 播放器与空失败态 | WGX-N4 | 与 LHY-N5/N6、LJQ-N5 并行 | 播放/下载/标识占位 | WGX-N6 |
| WGX-N6 对接后端零样本 | **LHY-N4 + LHY-N6** | 无；联调收口。LJQ 样音仅作对照，非硬依赖 | 浏览器验证记录 | **CXY-N6** |

> 前端口令：N1/N2 **不要等任何人**。真对接只卡两处——**上传等 LHY-N3，合成联调等 LHY-N4/N6**；标签文案等 CXY-N4。不要等算法训练，本周没有。

### 4.3 逐节点工作与提示词

#### WGX-N1 脚手架与信息架构

**对接关系：** **立刻开工，与另外三人彻底隔离。** 不等契约、不等 9880、不等音频。只对照开发草图与 Task1。
...
**对接关系：** 只硬等自己的 N1。状态枚举可先按 Task1 写，CXY-N3 冻结后对齐一次（软）。与全员前半周并行。把轮询/错误需求尽早软提给 LHY，但**不要停工空等回复**。
...
**对接关系：** 字段硬等 **CXY-N3**；真上传硬等 **LHY-N3**。此前可用本地 mock 把 UI 做完，**禁止空等后端每一天**。与 LJQ 数据准备隔离。不要直连 9880。
...
**对接关系：** 硬等 **CXY-N4**（标签与 5 条回归文本）。可与 LHY-N3/N4、LJQ-N4 并行；提交先接 mock，真 API 留到 N6。不等算法听测结论也能做壳。
...
**对接关系：** 只硬等自己的 N4。与 LHY-N5/N6、LJQ-N5 并行。可用本地短 wav 调播放器；**真 output URL 等到 N6**。不依赖 LJQ 韵律样音才能开发。
...
**对接关系：** **硬等 LHY-N4 + LHY-N6（OpenAPI）**。LJQ 样音仅作演示对照，非硬前置。这是前端本周收口，也是 CXY-N6 的硬证据之一。未接到真 API 前不要用前端假 wav 报完成。
...
| 产出 | 来源 | 接收方 | 等待类型 | 接收方何时才能继续 |
|---|---|---|---|---|
| 版本基线表模板 | CXY-N1 | LJQ、LHY | 硬（模板）/ 软（签字实测） | 有模板即可开始 N1；签字版用于环境验收 |
| 需求与证据矩阵 | CXY-N2 | 全员 | 软（他人）/ 硬（CXY-N3/N4） | 他人可不看继续干活；队长自己要等它写 N3/N4 |
| 接口契约 v0.1 | CXY-N3 | LHY、WGX | **硬** | LHY-N2/N3、WGX-N3 真对接前必须冻结 |
| 情感标签与 5 条回归文本 | CXY-N4 | LJQ、WGX | **硬** | LJQ-N4、WGX-N4 定稿前不要正式验收听测/控件 |
| 安全策略与授权文案 | CXY-N5 | LHY、LJQ、WGX | **软** | 可先开发；定稿后对齐授权字段与文案 |
| 官方 9880 可访问 | LJQ-N1 | LHY | **硬**（真代理） | LHY-N4 真实推理前；mock 单测可先行 |
| 授权音频与质量报告 | LJQ-N2 | CXY、LHY | 软给 LHY；硬给自己 N3 | 后端不挡；算法无数据则停 N3 |
| `.list` 与数据目录 | LJQ-N3 | LHY、CXY | 软（本周） | 本周不挡前后端；第 2 周变硬 |
| 零样本 3 次 WAV + 日志 | LJQ-N4 | CXY、LHY | **硬给 CXY-N6**；软给 LHY | 周五验收必须有；后端对照可选 |
| 韵律对照样音 | LJQ-N5 | CXY、WGX | 软/证据 | 不挡开发；周五建议有 |
| 算法交接文档 | LJQ-N6 | LHY、WGX、CXY | 软 | 不硬挡 LHY-N4；周五前齐 |
| 后端地址与 health | LHY-N1 | WGX | 软 | 前端可先 mock；有地址更好联调 |
| 上传 API | LHY-N3 | WGX | **硬**（真上传） | WGX-N3 真联调前；UI 壳可先行 |
| 零样本代理 TTS API | LHY-N4 | WGX、CXY | **硬** | WGX-N6、CXY-N6 |
| 任务状态 + OpenAPI 示例 | LHY-N5/N6 | WGX、CXY | **硬给 WGX-N6** | 无 OpenAPI 不宣称前端联调完成 |
| 浏览器验证记录 | WGX-N6 | CXY | **硬** | CXY-N6 |
| 本周验收结论 | CXY-N6 | 全员 | — | 本周终点 |
...
1. 先读 **§0.6**（等待矩阵与日历），确认自己会不会挡住别人、该不该空等。
2. 打开自己章节的节点总览表，再看每个节点下的 **对接关系** 一行。
3. 一次只复制一个节点的提示词；交付物落地后再进入下一节点。
4. 若提示词里的上游文件还不存在：属于硬等待则催对方或先做可并行节点；属于软依赖则按 Task1 草稿推进，定稿后对齐。
5. 禁止用假日志/Mock 音频填空来“解除”硬等待。