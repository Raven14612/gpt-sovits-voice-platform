...
工作内容不变，唯一变化是**上游只有 CXY 一个人**：契约 v2.0、合成参数、情绪选用规则、页面与跨页 State 全部由 CXY 交付。
...
- **哪些人的工作会影响我？** Week 2 `tts_service` 是硬依赖；CXY-N1、CXY-N2 是软依赖，收到后对齐。
- **我需要交付什么内容？** 合成服务、情绪样本写入接口、真实样例、测试与函数说明。
- **谁需要等待我完成该工作？** LHY-N3 等真实失败隔离；CXY-N3 等绑定；CXY-N4 等项目内验证入口。LHY-N2 可按冻结 schema 并行实现。
...
```text
请根据以下contract-v2.0和CXY-N2参数实现tts_service.synthesize：
<粘贴契约>
<粘贴参数与情绪规则>
...
- **谁需要等待我完成该工作？** LHY-N3 等失败隔离；CXY-N3 等历史 UI；CXY-N5 等抽检。
...
- **谁需要等待我完成该工作？** LHY-N4 等集成；CXY-N4/N5 等失败用例证据。
...
- 保存音色成功 → 刷新合成页音色下拉（按 CXY-N3 事件清单对齐）。
...
- **哪些人的工作会影响我？** LHY-N1/N2/N3 和 CXY-N3 都是硬依赖。
- **我需要交付什么内容？** 一键启动集成版、绑定表、启动说明和日志。
- **谁需要等待我完成该工作？** CXY-N5 等产品闭环预演；WGX-N1 等浏览器冒烟启动条件。
...
输入：<ui模块>、<tts_service/history_service/voice_service>、<当前app.py>、<CXY-N3 State>
