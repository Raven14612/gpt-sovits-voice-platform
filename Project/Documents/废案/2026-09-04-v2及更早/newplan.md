...
统一容量规则：单个节点原则上不超过 1.5 人日；周三中午前交付最小可集成产物，周四预演，周五只修阻断问题。阻塞超过半天必须写入 [Decisions.md](Decisions.md) 由 CXY 裁定降级或改派，不能等到周末才暴露。
...
> **v2 生效（2026-09-03）**：WGX、LJQ 其他事务较多，改为 **CXY 主导 + 单点决策**。  
> 角色模型、负载表与决策机制真值：[Team Plan v2 (CXY-Led).md](Team%20Plan%20v2%20(CXY-Led).md)；决议台账：[Decisions.md](Decisions.md)。  
> 关键路径只有 CXY 与 LHY；LJQ、WGX 的产物全部为非阻断补充。

### CXY：核心开发与唯一决策者

- 对范围、契约冻结、每周 Demo 和最终验收负责；**决议写入台账即生效，不需他人确认**。
- 实现 Gradio 前端全部内容：顶栏、侧栏、四个独立页面、页面显隐、共享状态、跨页刷新/跳转/回填、主题与四态。
- 承担算法主轨：环境打通、GPT-SoVITS 运行、切分/ASR 与零样本推理、`tts_service` 参数与情绪映射、回归与轻量对照实验。
- 维护授权清单、固定文本、听测表、风险表、功能—证据矩阵与环境矩阵。
- 编写 README、演示脚本、录屏分镜与答辩 PPT。
- 禁止把计划功能写成已实现，禁止恢复训练、水印和 SaaS 功能；不修改核心模型，不承诺模型级解耦或准确率百分比。

### LHY：Python 业务与持久化（负载不变）

- 实现 services、schemas、文件校验、路径限制、日志和原子 JSON。
- 负责 `app.py` 启动集成、Gradio queue 和失败不写成功记录。
- 编写非 GPU 单元测试；Week 4 负责失败加固、全新实例验证与 README 技术章节。
- 上游只有 CXY 一人；答辩彩排时负责现场技术应急。
- 不增加 FastAPI、数据库或另一套任务系统。

### LJQ：轻量算法辅助（大幅减负，1–2 节点/周）

- 备用机环境记录与交叉验证；不参与主轨环境。
- 授权与情绪素材收集，附来源与授权说明。
- 人工听测（描述性判断，不给百分比）。
- 答辩前资源检查与算法边界复核意见。
- 不写代码，不进关键路径；缺席时由 CXY 兜底。

### WGX：轻量交付辅助（大幅减负，1–2 节点/周）

- 浏览器冒烟检查与流程截图归档。
- 浏览器兼容验证（Chrome / Edge、缩放、播放与下载）。
- 按分镜录屏与终稿截图。
- 只登记问题、不修改代码；不进关键路径；缺席时由 CXY 兜底。

### 协作纪律

1. **单点决策**：CXY 起草 → 审查 → 拍板 → 写入 [Decisions.md](Decisions.md) → 广播。取消多方确认环节。
2. **禁止讨论会**：异议只能以单条书面问题提交（一句话问题 + 一句话影响），CXY 24 小时内书面裁定，裁定即终结。
3. **交接唯一格式**：`[节点ID] 产物路径 + 一行说明 + 已知限制`；接收人不需回复确认。
4. Week 1 周中完成契约草案，Week 2 首次保存音色前定稿 v1.0，Week 3 首次写入历史前定稿 v2.0。
5. 每周四做集成预演；周五不合并新功能，只修阻断项并验收。
6. 一次只运行一个 GPU 重任务；主轨为 CXY 本机（Track B，8 GB 显存）。
