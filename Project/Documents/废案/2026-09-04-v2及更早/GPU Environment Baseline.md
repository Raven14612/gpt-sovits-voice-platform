...
> 适用对象：全组四人（**CXY 主责维护主轨与矩阵**，LJQ 提供备用轨道交叉验证，LHY/WGX 只读引用）  
> 用途：说明 RTX 40 系与 50 系混用时的协作规则，并提供每人可填写的环境基线模板。  
> 关联节点：Week 1 `CXY-N3`（主轨打通）与 `LJQ-N1`（备用轨道，非阻断）；Week 2 及以后 GPU 冒烟前对照本表。
...
CXY 负责打通主轨（本机 Track B）并维护下文「全组环境矩阵」；LJQ 在自己机器上做一次备用轨道记录作为交叉验证（非阻断）。LHY 的 `tts_service` 只依赖稳定 Python 接口，**不得写死某一张卡的 CUDA 版本**。
...
复制本节到 `Project/Documents/env/<姓名缩写>-baseline.md`（例如 `env/CXY-baseline.md`），由 CXY 汇总到全组矩阵。
...
|---|---|---|---|---|---|---|---|---|---|
| **CXY（主轨 / 主 Demo 机）** | RTX 5060 Laptop GPU (8 GB) | Track B | 610.88 | 3.13.11（待换 3.11） | 未安装 | 未测 | `GPT-SoVITS-main` 工作区源码；50系包存档未运行 | 未测/阻塞 | 2026-09-02 |
| LJQ（备用轨道，非阻断） | | | | | | | | | |
| LHY | 不承担模型执行 | — | — | — | — | — | — | 不要求 | — |
| WGX | 不承担模型执行 | — | — | — | — | — | — | 不要求 | — |

> 上表 CXY 行原为 `env/LJQ-baseline.md` 中由 CXY 代跑的记录；v2 起该机器即主轨，四项 P0 阻塞（PyTorch、权重、FFmpeg、ASR）归 `CXY-N3`。

**验收口径：**

- Week 1：**主 Demo 机（CXY，Track B）** 必须在本轨道 TTS 出 1 条真实 WAV；矩阵中 CXY 行完整。
- LJQ 备用轨道记录为**非阻断补充**；LHY、WGX 不承担模型执行，无需环境记录。
- Week 2+：环境变更（换包、换 PyTorch、换机器）后 **24 小时内** 更新个人表与矩阵。
- **不要求** 四人 PyTorch 版本完全一致；**要求** 业务代码 + 模型版本一致，且主轨 GPU 链路可证明。
...
| 官方 WebUI 能跑、`app.py` 不能 | 项目入口或路径未对齐 | LHY 对照 CXY-N3 的环境记录修导入路径，不是换驱动 |

---

## 8. 与阶段 Demo 的关系

- **答辩主 Demo**：默认使用 **CXY 本机（Track B）**；除现场故障外不更换。
- **备用机（LJQ 侧）**：若已完成交叉验证，提供本机 WAV + 日志作为降级素材与旁证。
...
| 角色 | 责任 |
|---|---|
| **CXY** | 打通并维护主轨环境、维护双轨道说明与全组矩阵、验收证据是否本机真实产出、环境变更时通知 LHY |
| **LJQ** | 在自己机器上完成一次备用轨道记录与交叉验证（非阻断）；答辩前资源检查 |
| **LHY** | 读模型路径与入口，不在代码里写死 CUDA 版本 |
| **WGX** | 不承担环境工作；浏览器冒烟时如实登记环境状态区显示结果 |
...
- 协作模式：`Project/Documents/Team Plan v2 (CXY-Led).md`
- Week 1 节点：`Project/Documents/Week1/WorkScheduleGuide/CXY Nodes.md`（CXY-N3 主轨）、`LJQ Nodes.md`（LJQ-N1 备用轨道）
