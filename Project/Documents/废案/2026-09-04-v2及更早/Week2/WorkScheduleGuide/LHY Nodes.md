...
工作内容不变，唯一变化是**上游只有 CXY 一个人**：契约、参考规范、算法参数、页面与跨页 State 全部由 CXY 交付。
...
- **哪些人的工作会影响我？** Week 1 服务骨架是硬依赖；CXY-N1、CXY-N2 是软依赖，收到后对齐字段和参数。
- **我需要交付什么内容？** 完整音频服务、真实数据集样例、测试与函数说明。
- **谁需要等待我完成该工作？** CXY-N3 等真实返回结构；LHY-N2/LHY-N4 等参考片段；CXY-N5 等样例抽检。
...
- **哪些人的工作会影响我？** 合法短参考与 Week 1 `tts_service` 骨架是启动依赖；契约 v1.0 在首次正式保存前是硬依赖；LHY-N1 只影响完整跨页接入，CXY-N2 参数是软依赖。
- **我需要交付什么内容？** `generate_preview`、`save_voice`、真实试听和接口说明。
- **谁需要等待我完成该工作？** LHY-N3 等保存目录；CXY-N3 等事件返回与项目内试听入口。LHY-N1 不等待本节点，可并行推进音频处理。
...
输入：<contract-v1.0 voice部分>、<CXY-N2试听参数>。
...
- **谁需要等待我完成该工作？** LHY-N4 等列表能力；CXY-N5 等档案与恢复证据。
...
- **哪些人的工作会影响我？** LHY-N1/N2/N3 和 CXY-N4 都是硬依赖。
- **我需要交付什么内容？** 一键启动集成版、绑定表、启动说明和日志。
- **谁需要等待我完成该工作？** CXY-N5 等阶段预演；WGX-N1 等浏览器冒烟启动条件。
