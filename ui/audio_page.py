...
def load_transcript(dataset_id, path):
    try:
        ...
        return dataset_service.load_corrections(dataset_id), None, "识别文本已导入，可直接确认使用，也可展开校对面板修改。"
    except (AppError, OSError) as exc:
        return gr.update(), None, error_text(exc)


def save_table(dataset_id, rows, annotation_name=""):
    try:
        dataset_service.save_corrections(dataset_id, rows, annotation_name=annotation_name)
        return "标注和情绪标签已保存，可前往「音色训练与仓库」准备训练数据。"
    except (AppError, OSError) as exc:
...
def render_audio_page(state):
    ...
    with gr.Accordion("导入音频", open=True):
        ...
        name = gr.Textbox(label="数据集名称（支持中文）", max_length=80)
        upload = gr.Button("保存音频", variant="primary", elem_id="hint-audio-upload")

    # The saved dataset becomes selectable only after the import/name/save flow.
    # Keeping this block directly below the importer makes that lifecycle visible.
    with gr.Row():
        datasets = gr.Dropdown(label="当前数据集", choices=dataset_choices(), value=None, interactive=True)
        refresh = gr.Button("刷新数据集", size="sm", scale=0, elem_id="hint-audio-refresh")

    source = gr.Textbox(label="已保存音频路径", interactive=False, elem_classes=["rj-readout"])
    with gr.Row():
        saved_name = gr.Textbox(label="修改数据集名称（支持中文）", max_length=80)
        rename = gr.Button("保存数据集名称", elem_id="hint-audio-rename")
    with gr.Row():
    ...
    with gr.Accordion("识别与人工校对（可选）", open=False, elem_id="slice-corrections"):
        ...
        table = gr.Dataframe(
            ...
            label="切片标注", interactive=False,
            column_widths=["35%", "45%", "20%"], wrap=True,
        )
        selected_path = gr.Textbox(label="选中切片", interactive=False, elem_classes=["rj-readout"])
        text = gr.Textbox(label="校对文本", lines=3)
        emotion = gr.Radio(label="人工情绪标签", choices=["neutral", "happy", "sad"], value="neutral")
        annotation_name = gr.Textbox(label="标注版本名称（支持中文）", placeholder="例如：能天使·人工校对第一版", max_length=80)
        apply = gr.Button("更新切片", elem_id="hint-audio-apply")
        gr.Markdown("修改后先点击「更新切片」，再点击面板外的「确认标注并用于训练」保存。")
        export = gr.Button("导出中文名称标注", elem_id="hint-audio-export")
        exported = gr.File(label="下载标注", interactive=False)
    save = gr.Button("确认标注并用于训练", variant="primary", elem_id="hint-audio-save")
    ...
    datasets.change(lambda value: ((r.display_name, r.annotation_name, None) if
                    (r := dataset_service.get_dataset(value)) else ("", "", None)), datasets,
                    [saved_name, annotation_name, exported])
    def rename_selected(value, title):
        try:
            dataset_service.rename_dataset(value, title)
            return gr.update(choices=dataset_choices(), value=value), "数据集名称已保存。"
        except AppError as exc:
            return gr.skip(), error_text(exc)
    def export_selected(value):
        try:
            return dataset_service.export_transcript(value), "标注已导出。"
        except (AppError, OSError) as exc:
            return None, error_text(exc)
    rename.click(rename_selected, [datasets, saved_name], [datasets, message])
    export.click(export_selected, datasets, [exported, message])
